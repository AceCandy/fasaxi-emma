...
public class VideoStaticUtil {
    ...
    private byte[] downloadRange(String url, long start, long end) {
        try (Response resp = HttpUtil.send(Request.of(url).method(Method.GET)
    ...
    private long getFileSize(String url) {
        try (Response resp = HttpUtil.send(Request.of(url).method(Method.HEAD).setMaxRedirects(1))) {
