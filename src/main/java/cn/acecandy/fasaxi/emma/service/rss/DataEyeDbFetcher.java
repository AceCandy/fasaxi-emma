...
public class DataEyeDbFetcher {
     ...
     * 固定Top30数量
     */
    private static final int TOP_NUM = 30;

    /**
     * 核心处理
     * <p>
     * SdJccRank=4:month:2025 年度:所有月份的item相加 按重复次数排行 超过30保留top30 不超过30补充最后一月剩下的item
     * SdJccRank=4:day:thisMonth 月度:该月所有item相加 按重复次数排行 超过30保留top30 不超过30补充最后一天剩下的item
     *
     * @param url 路径
     * @return {@link List }<{@link MatchedItem }>
     */
    public Set<String> exec(String url) {
        String keyword = StrUtil.removePrefix(url, "SdJccRank=");
        List<String> keywords = SplitUtil.split(keyword, ":");
        Integer type = Integer.parseInt(CollUtil.getFirst(keywords));
        String timeStage = CollUtil.get(keywords, 1);
        String timeValue = CollUtil.get(keywords, 2);

        if (StrUtil.equals(timeValue, "thismonth")) {
            timeValue = DateFormatPool.NORM_MONTH_FORMAT.format(DateUtil.lastMonth());
        }


        List<SdJccRank> ranks = sdJccRankDao.findByTypeAndTimeStageLike(type, timeStage, timeValue);
        if (CollUtil.isEmpty(ranks)) {
            return SetUtil.ofLinked();
        }
        // 步骤3：内存统计→解析所有JSON，统计短剧ID出现频次（核心步骤）
        Map<Long, Integer> frequencyMap = statisticsDramaIdFrequency(ranks);
        if (MapUtil.isEmpty(frequencyMap)) {
            return SetUtil.ofLinked();
        }

        // 步骤4：筛选TopN→按频次降序排序，超30截前30，不足保留原数据
        List<Long> topDramaIds = filterTopDramaIds(frequencyMap);

        // 步骤5：补全至30→不足30时，从最后时间维度补充未上榜的短剧ID
        if (CollUtil.size(topDramaIds) < TOP_NUM) {
            topDramaIds = supplementTo30(topDramaIds, ranks);
        }

        List<SdItem> top30 = sdItemDao.findBySourceId(topDramaIds);
